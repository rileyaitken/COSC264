...
int main(int argc, char **argv)
{
    ...
    int s_in_sock, s_out_sock, cs_in_sock = 0;
    //int s_in_conn = 0;
    //int s_in_addr_len = 0;
    ...
    if ((cs_in_sock = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
        error("Error creating the s_in socket.");
    }
    ...
    if ((n = bindSocket(cs_in_sock, &cs_in_addr, s_in_portno)) < 0) {
        error("Error binding s_in socket to its address.");
    } 
    
    /* Connect the s_out socket to cs_in */
    connectSocket(s_out_sock, &s_out_addr, cs_in_portno);
